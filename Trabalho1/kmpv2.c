...
static void compute_centroids(void* thread_id) {
	int min = (((int) thread_id) * ncentroids)/nthreads;
	int max = ((((int) thread_id)+1) * ncentroids)/nthreads;
...
static void populate(void *thread_id) {
		int i, j;
		float tmp;
		float distance;
		int min = (((int) thread_id) * npoints)/nthreads;
		int max = ((((int) thread_id)+1) * npoints)/nthreads;
...
static void compute() {
	...
	for (i = 0; i < nthreads; i++) {
		pthread_create(&threads[i], NULL, populate, i);
	}
...
int main (int argc, char** argv) {
	...
	if (argc != 7) {
		printf ("Usage: npoints dimension ncentroids mindistance seed maxthreads\n");
		exit(1);
	}
